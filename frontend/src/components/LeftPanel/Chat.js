...
const Chat = ({name,handleClick,online,chat}) => {
  ...
  return (
    ...
    {name}{chat[0]?chat[0].sender===name?<div className='ml-3'>{chat[0].type==='text'?chat[0].message:'File'}</div>:<div className='ml-3'>you : {chat[0].type==='text'?chat[0].message:'File'}</div>:''}
