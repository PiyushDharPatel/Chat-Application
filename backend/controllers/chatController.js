...
const addchat=asyncHandler(async(req,res)=>{
    const {user1,user2,message,read,type}=req.body
    let date=(new Date())
    ...
    ml1.push({sender:user1,message:message,read:read,date:date,type:type})
    chat1.overwrite(Object.assign(chat1.toObject(),{chats:ml1}))
    const ml2=chat2.chats
    ml2.push({sender:user1,message:message,read:read,date:date,type:type})
...
const getchat=asyncHandler(async(req,res)=>{
    ...
    chat2.chats.forEach((item)=>{
            ...
            cl1.push({sender:item.sender,message:item.message,read:true,date:item.date,type:item.type})
